...
import { Layer, Stage, Transformer, Rect, Circle, RegularPolygon, Line, Arrow, Image, } from "react-konva";
...
function Canvas() {
    ...
    function handleEraser(event) {
        ...
        dispatch({
            ...
            payload: {
                ...state,
                selectedShapes: state.selectedShapes.filter((_, index) => index !== event.target.index),
            },
        });
        // setSelectedIds(
    ...
    function Renderer(shapes) {
        return shapes.map((struct, index) => {
            if (struct.shapeName === "rect") {
                ...
                return (_jsx(RegularPolygon, { ref: (node) => {
                        if (node) {
                            shapeRefs.current.set(index, node);
                        }
                    }, ...struct }));
                ...
                return (_jsx(Arrow, { ref: (node) => {
                        if (node) {
                            shapeRefs.current.set(index, node);
                        }
                    }, points: struct.points, ...struct }));
            }
            else if (struct.shapeName === "image" && struct.image) {
                return (_jsx(Image, { ref: (node) => {
                        if (node) {
                            node.cache();
                            shapeRefs.current.set(index, node);
                        }
    ...
    return (_jsxs(TransformWrapper, { minScale: 1, onTransformed: (event) => setZoomLevel(event.state.scale), panning: { disabled: pan }, children: [_jsx(Zoomer, { zoomLevel: zoomLevel }), _jsx(TransformComponent, { children: _jsx(Stage, { ref: canvasRef, onClick: handleStageClick, onTap: handleStageClick, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onTouchStart: handleMouseDown, onTouchMove: handleMouseMove, onTouchEnd: handleMouseUp, width: window.innerWidth, height: window.innerHeight, children: _jsxs(Layer, { children: [Renderer(state.structures), lines.map((line, i) => (_jsx(Line, { points: line, index: i, ref: (node) => {
                                    if (node) {
                                        shapeRefs.current.set(state.structures.length + i, node);
                                    }
                                }, stroke: "#008140", draggable: true, strokeWidth: 3, tension: 0.2, lineCap: "round", lineJoin: "bevel" }, state.structures.length + i))), _jsx(Transformer, { ref: trRef, 
                                // padding={6}
                                centeredScaling: false, rotationSnapTolerance: 100, rotateAnchorOffset: 20, rotateLineVisible: false, anchorSize: 6, borderStroke: "#008140", anchorCornerRadius: 6, anchorFill: "#05df72", anchorStroke: "#05df72" })] }) }) })] }));
}
